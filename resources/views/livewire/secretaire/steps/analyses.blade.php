            ...
            <li class="list-group-item d-flex justify-content-between align-items-center">
                <div>
                    ...
                    @if($analyse['level'] === 'PARENT')
                        <span class="fw-bold">{{ $analyse['designation'] }}</span>
                    @else
                        <span class="ms-4">{{ $analyse['designation'] }}</span>
                    @endif
                </div>
                <span>
                    {{ number_format($analyse['prix'], 2) }} Ariary
                    <button type="button" class="btn btn-sm btn-danger ms-2" wire:click="removeAnalyse({{ $analyseId }})">
